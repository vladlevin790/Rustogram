...
export default function Post({ post, onLikeClick, likesData, user }) {
  ...
  return (
    <div className="flex flex-col w-[770px] h-[975px] bg-[#F9F8F8] border border-gray-300 rounded-md shadow-md mb-4 relative">
      ...
      <div className="flex flex-col items-center w-full">
        ...
        <div className="flex mt-5 w-full ml-8 font-semibold text-[20px]">
          <h2>{post.likes} {formatLikesWord(post.likes)}</h2>
        </div>
        <div className="w-full flex items-center cursor-pointer">
          <Burger className="w-6 ml-4 mt-3" />
          <h2 className="font-bold" onClick={handleCommentButtonClick}>Открыть комментарии</h2>
        </div>
      </div>
      )}
      {isComment && (
        <div className="flex flex-col items-center w-full">
          <div className="flex justify-between ml-2 w-[740px] mt-5">
            <Link to="/user_profile" className="flex items-center gap-2 w-full ml-6">
              {isAvatar ? <img src={post.user.avatar} alt=""/> :
